...
| P2 Dominic | Fusion & Demo Lead | FSVG, visual encoder, web demo |
| P3 Zhenning | Backbone & Data Lead | Mamba modification, lite codec, video pipeline |
...
|   |-- mamba_block.py            original blocks + new CausalTFMambaBlock (Zhenning)
...
|   |-- lite_visual_encoder.py    new, MobileNetV3 per-frame + 3D CNN temporal (Dominic)
...
|-- train_lite.py                 our training loop, no discriminator (Ronny + Fan)
...
| `dataloaders/dataloader_av.py` audio | Fan | mix_audio, RMS normalization, noise loading, STFT pipeline |
| `dataloaders/dataloader_av.py` video | Zhenning | load_video_frames with OpenCV, apply_visual_augmentation |
...
| `models/lite_visual_encoder.py` | Dominic | EncoderA pretrained backbone + EncoderB custom 3D CNN |
| `models/mamba_block.py` | Zhenning | Fix mamba-ssm 2.3.0 imports, add bidirectional param, Mamba-2 switch, CausalTFMambaBlock |
| `models/codec_module.py` | Zhenning | DepthwiseSeparableConv2d, LiteDenseBlock, LiteDenseEncoder, LiteMagDecoder, LitePhaseDecoder |
| `models/loss.py` si_sdr | Ronny | si_sdr_loss, si_sdr_score |
| `models/loss.py` stoi | Shunjie | stoi_score |
